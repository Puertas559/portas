(() => {
  ...
  $("runBulkSiteAnalysis")?.addEventListener("click",async()=>{
    ...
    let done=0, failed=0;
    $("bulkSiteMessage").textContent=`Iniciando Quick Scan de ${urls.length} sitios · hasta 4 análisis en paralelo…`;
    const queue=[...urls];
    ...
    try{await Promise.all(Array.from({length:Math.min(4,urls.length)},worker));toastLocal("Quick Scan en lote terminado");}
