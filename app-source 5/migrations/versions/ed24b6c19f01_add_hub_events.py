...
def upgrade():
    # Idempotent safeguard: older HUB test deployments may already have created
    # one or more tables before Alembic recorded this revision. In that case,
    # do not take the whole Radar offline with a duplicate-table error.
    bind = op.get_bind()
    inspector = sa.inspect(bind)
    existing = set(inspector.get_table_names())

    if 'hub_event_sources' not in existing:
        ...
        op.create_index('ix_hub_event_sources_tenant_id','hub_event_sources',['tenant_id'])
        op.create_index('ix_hub_event_sources_status','hub_event_sources',['status'])

    if 'hub_events' not in existing:
        ...
        for col in ['tenant_id','source_id','name','normalized_key','source_mode','status','total_score','created_at']:
            op.create_index(f'ix_hub_events_{col}','hub_events',[col])

    if 'hub_event_accounts' not in existing:
        ...
        for col in ['tenant_id','event_id','company_id','company_name','tier','status']:
            op.create_index(f'ix_hub_event_accounts_{col}','hub_event_accounts',[col])

    if 'hub_event_actions' not in existing:
        ...
        for col in ['tenant_id','event_id','phase','due_at','status']:
            op.create_index(f'ix_hub_event_actions_{col}','hub_event_actions',[col])
