(() => {
  ...
  function renderRows() {
    ...
    box.innerHTML = state.rows.map((row) => {
      ...
      return `<article class="map-place-card ${checked ? "selected" : ""} ${row.inCrm ? "in-crm" : ""}" data-place-id="${esc(row.placeId)}">
        ...
        <div class="map-place-meta"><span>${esc(row.primaryType || "Empresa")}</span><span>${esc(rating)}</span><span class="term-badge">${esc(row.matchedTerm || "búsqueda")}</span>${row.inCrm ? '<span class="crm-badge">YA EN CRM</span>' : ""}</div>
        <div class="map-place-actions">${row.website ? `<a href="${esc(row.website)}" target="_blank" rel="noopener">Sitio</a>` : ""}${row.mapsUrl ? `<a href="${esc(row.mapsUrl)}" target="_blank" rel="noopener">Maps</a>` : ""}${row.phone ? `<button type="button" data-copy-phone="${esc(row.phone)}">${esc(row.phone)}</button>` : ""}</div></div>
      </article>`;
    }).join("");
  ...
  async function runSearch() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "No se pudo consultar Google Places");
      state.rows = data.results || []; state.selected.clear();
      const newCount = state.rows.filter((r) => !r.inCrm).length;
      byId("mapSearchStatus").textContent = `${data.count} empresas únicas · ${newCount} nuevas · ${data.calls} consultas Google · ${data.queries.length} términos expandidos.`;
      renderRows();
