...
from ..services.hub_events import (
    analyze_event_account, apply_automatic_intelligence, build_playbook, create_detected_event,
    event_key, extract_company_candidates_from_url, extract_page, parse_company_text, run_hub_event_scan
)
...
def overview():
    ...
    accounts=HubEventAccount.query.filter_by(tenant_id=t.id).all()
    return jsonify(events=[x.to_dict() for x in rows], metrics={
        'detected': sum(x.status in {'DETECTED','TRIAGE','VALIDATED','ANALYSIS','MONITOR'} for x in rows), 'active': len(active),
        ...
        'accounts': len(accounts), 'analyzedAccounts': sum(x.status == 'ANALYZED' for x in accounts),
    })
...
def import_url():
    ...
    except Exception as exc: return jsonify(error=f'Não foi possível ler a página: {exc}'),422
...
def update_event(event_id):
    ...
    if 'status' in d:
        ...
        if d['status'] not in allowed: return jsonify(error='Status inválido'),400
        row.status=d['status']
...
def reanalyze_event(event_id):
    ...
    if not row.url: return jsonify(error='Este evento não possui URL para reanálise automática.'),400
    try: info=extract_page(row.url)
    except Exception as exc: return jsonify(error=f'Não foi possível reanalisar a página: {exc}'),422
    apply_automatic_intelligence(row, info, create_accounts=True)
...
def add_account(event_id):
    ...
    if not name:return jsonify(error='Empresa obrigatória'),400
...
@hub_bp.post('/api/events/<int:event_id>/accounts/import-text')
@require_permission('WRITE_CRM')
def import_accounts_text(event_id):
    row=_event_or_404(event_id); d=request.get_json(silent=True) or {}
    items=parse_company_text(d.get('text') or '')
    if not items: return jsonify(error='Nenhuma empresa válida foi encontrada no texto.'),400
    created=duplicates=0
    for item in items:
        name=item['companyName']; website=item.get('website')
        existing=HubEventAccount.query.filter_by(tenant_id=row.tenant_id,event_id=row.id,company_name=name).first()
        if existing: duplicates+=1; continue
        db.session.add(HubEventAccount(tenant_id=row.tenant_id,event_id=row.id,company_name=name,website=website,role='PARTICIPANT',tier='C',icp_score=0,status='MAPPED')); created+=1
    db.session.commit(); return jsonify(created=created,duplicates=duplicates,total=len(items))

@hub_bp.post('/api/events/<int:event_id>/accounts/discover-url')
@require_permission('WRITE_CRM')
def discover_accounts_url(event_id):
    row=_event_or_404(event_id); d=request.get_json(silent=True) or {}; url=(d.get('url') or row.url or '').strip()
    if not url: return jsonify(error='Informe a URL da lista de expositores/participantes.'),400
    try: items=extract_company_candidates_from_url(url)
    except Exception as exc: return jsonify(error=f'Não foi possível ler a lista de empresas: {exc}'),422
    created=duplicates=0
    for item in items:
        name=item['companyName']; website=item.get('website')
        existing=HubEventAccount.query.filter_by(tenant_id=row.tenant_id,event_id=row.id,company_name=name).first()
        if existing: duplicates+=1; continue
        db.session.add(HubEventAccount(tenant_id=row.tenant_id,event_id=row.id,company_name=name,website=website,role=item.get('role') or 'PARTICIPANT',tier='C',icp_score=0,status='MAPPED')); created+=1
    db.session.commit(); return jsonify(created=created,duplicates=duplicates,found=len(items))

@hub_bp.post('/api/events/<int:event_id>/accounts/analyze')
@require_permission('WRITE_CRM')
def analyze_accounts(event_id):
    row=_event_or_404(event_id); d=request.get_json(silent=True) or {}; requested=d.get('accountIds') or []
    q=HubEventAccount.query.filter_by(tenant_id=row.tenant_id,event_id=row.id)
    if requested: q=q.filter(HubEventAccount.id.in_([int(x) for x in requested]))
    else: q=q.filter(HubEventAccount.website.isnot(None), HubEventAccount.status.in_(['MAPPED','ANALYSIS_ERROR']))
    accounts=q.order_by(HubEventAccount.id.asc()).limit(8).all()
    if not accounts: return jsonify(analyzed=0,success=0,errors=0,remaining=0,message='Nenhuma empresa com site pendente de análise neste lote.')
    results=[]; success=errors=0
    for account in accounts:
        result=analyze_event_account(account,max_pages=3); results.append({'id':account.id,'companyName':account.company_name,**result})
        success+=int(result.get('ok')); errors+=int(not result.get('ok'))
    db.session.commit()
    remaining=HubEventAccount.query.filter_by(tenant_id=row.tenant_id,event_id=row.id).filter(HubEventAccount.website.isnot(None),HubEventAccount.status.in_(['MAPPED','ANALYSIS_ERROR'])).count()
...
def add_source():
    ...
    if not url or not name:return jsonify(error='Nome e URL obrigatórios'),400
