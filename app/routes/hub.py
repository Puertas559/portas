...
def import_url():
    ...
    row,created=create_detected_event(current_tenant().id,info['name'],url=url,source_mode='URL',description=info['description'],confidence=70)
    db.session.commit(); return jsonify(event=row.to_dict(),created=created)
...
def score_event(event_id):
    ...
    row.score_details=d; row.status='ANALYSIS' if row.status in {'DETECTED','TRIAGE','VALIDATED'} else row.status
...
def scan_sources():
    if not _allowed():abort(404)
    sources=HubEventSource.query.filter_by(tenant_id=current_tenant().id,status='ACTIVE').all();created=0;found=0;errors=[]
    for source in sources:
        try:
            links=scan_source(source);found+=len(links)
            for url,label in links:
                _,is_new=create_detected_event(current_tenant().id,label,url=url,source=source,source_mode='AUTOMATIC',confidence=60);created+=int(is_new)
        except Exception as exc:
            source.last_error=str(exc)[:500];errors.append({'source':source.name,'error':str(exc)})
    db.session.commit();return jsonify(sources=len(sources),found=found,created=created,errors=errors)
