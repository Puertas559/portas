(() => {
  ...
  const moduleTitles = {
    triage:"Calificar por sitio", research:"Research Queue", salesready:"Sales Ready", hoy:"Mi día",
    crm:"CRM", pipeline:"Pipeline", visitas:"Visitas", radar:"Radar comercial", captacion:"Captación automática",
  ...
  function openModule(name){
    ...
    if ($("workspaceModuleTitle")) $("workspaceModuleTitle").textContent = moduleTitles[name] || "Sales Workspace";
  ...
  function readinessCard(lead){
    ...
    return `<article class="readiness-card ${bulkSelection.has(String(lead.id)) ? "selected-card" : ""}" data-ready-id="${esc(lead.id)}">
      ...
      <div><span class="${ready?"ready-flag":"blocked-flag"}">${ready?"● SALES READY":"○ AÚN NO LISTO"}</span><h3>${esc(lead.company)}</h3><p>${esc(lead.project || "Proyecto por validar")}</p>
      <div class="readiness-meta"><span>Readiness ${Number(lead.leadReadiness)||0}</span><span>Score ${Number(lead.score)||0}</span><span>Conf. ${Number(lead.confidenceScore)||0}</span><span>Datos ${Number(lead.dataCompleteness)||0}%</span></div>
  ...
  function renderWorkspace(data){
    ...
    if ($("researchTable")) $("researchTable").innerHTML = research.map((row)=>`<article class="work-row">
      ...
      <div><strong>Fit ${row.fit||0}</strong><small>Momentum ${row.momentum||0}</small></div>
    ...
    if ($("salesReadyList")) $("salesReadyList").innerHTML = (data.salesReady||[]).map(readinessCard).join("") || "<p>No hay oportunidades cercanas a Sales Ready.</p>";
  ...
  async function loadWorkspace(force=false){
    ...
    catch(_){ toastLocal("No se pudo cargar el workspace comercial"); }
  }
  ...
  function renderSmartList(key){
    ...
    $("smartListResults").innerHTML=rows.map((row)=>{
      if(row.companyId && !row.id) return `<article class="readiness-card"><div class="readiness-score">${Number(row.completeness)||0}</div><div><h3>${esc(row.company)}</h3><p>Datos vencidos o incompletos · Fit ${Number(row.fit)||0}</p></div></article>`;
  ...
  document.querySelectorAll("[data-bulk-action]").forEach((b)=>b.addEventListener("click",async()=>{
    if(!bulkSelection.size) return toastLocal("Seleccione oportunidades en Sales Ready");
  ...
  $("runBulkSiteAnalysis")?.addEventListener("click",async()=>{
    ...
    const button=$("runBulkSiteAnalysis"); button.disabled=true; $("bulkSiteMessage").textContent="Analizando sitios en lote. Esto puede tardar algunos segundos...";
    try{const r=await fetch("/api/website-analysis/bulk",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({urls:raw})});const d=await r.json();if(!r.ok&&r.status!==207)throw new Error(d.error||"Error en lote");
      (d.results||[]).reverse().forEach((analysis)=>$("siteAnalysisResults")?.insertAdjacentHTML("afterbegin",typeof renderWebsiteAnalysis==="function"?renderWebsiteAnalysis(analysis):""));
      $("bulkSiteMessage").textContent=`${d.analyzed||0} analizadas · ${d.failed||0} con error.`; toastLocal("Análisis en lote terminado");
    }catch(err){$("bulkSiteMessage").textContent=err.message}finally{button.disabled=false}
  });
  ...
  async function loadCommittee(){
    ...
    try{
      ...
      $("committeeList").innerHTML=(rows||[]).map((c)=>`<article class="committee-contact"><strong>${esc(c.name)}</strong><span>${esc(c.role||"Cargo por validar")}</span><small>${esc(c.email||c.whatsapp||c.phone||"Sin contacto directo")}</small><em>${esc(c.buyingRole||"UNKNOWN")} · influencia ${Number(c.influence)||0}</em></article>`).join("")||"<p>No hay contactos todavía. Añada el primer decisor o influenciador.</p>";
  ...
  $("saveContact")?.addEventListener("click",async()=>{const companyId=$("committeeList")?.dataset.companyId;if(!companyId)return toastLocal("Abra primero una oportunidad real");const name=$("contactName").value.trim();if(!name)return toastLocal("Ingrese el nombre");const phone=$("contactPhone").value.trim();const payload={name,role:$("contactRole").value.trim(),buyingRole:$("contactBuyingRole").value,email:$("contactEmail").value.trim(),phone,whatsapp:phone,influence:$("contactBuyingRole").value==="DECISION_MAKER"?90:70,confidence:70};const r=await fetch(`/api/companies/${companyId}/contacts`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});if(!r.ok)return toastLocal("No se pudo guardar el contacto");["contactName","contactRole","contactEmail","contactPhone"].forEach((id)=>$(id).value="");toastLocal("Contacto añadido al Buying Committee");loadCommittee();loadWorkspace(true)});

  // Keep the drawer intelligence indicators synchronized whenever a lead is selected.
  const syncDrawer=()=>{ if(!selected)return; if($("drawerReadiness"))$("drawerReadiness").textContent=`${Number(selected.leadReadiness)||0}/100`;if($("drawerCompleteness"))$("drawerCompleteness").textContent=`${Number(selected.dataCompleteness)||0}%`;if($("drawerSalesReady")){$("drawerSalesReady").textContent=selected.salesReady?"SALES READY":"NO LISTO";$("drawerSalesReady").classList.toggle("ready",Boolean(selected.salesReady))}if($("nextBestActionText"))$("nextBestActionText").textContent=selected.nextBestAction||"Completar contacto y validar el próximo paso.";};
